...
node test/content.test.mjs  # panel rendering over a stubbed DOM (no-bid gate, late-bid hydration)
...
failure. Run all three files; there is no way to run a single named test.
...
- **`boot()` is invoked at the very end of the content.js IIFE.** The storage callback can
  ...
  `let root, host`.
- **No bid known → no dollar figure.** With no bid parsed or typed, the cost engines
  happily price a £0/A$0 hammer and the panel showed fees-only totals as if real
  ("A$14 delivered" on AWA browse pages — just default shipping). `scenariosHTML()`
  gates on `listing.bid == null` and renders an enter-your-bid prompt instead; the
  gate lives there so the focus-preserving `liveRecompute()` path honours it too.
