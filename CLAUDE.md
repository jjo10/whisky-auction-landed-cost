...
Releases: bump `manifest.json` version, commit, then `git tag v<version> && git push
origin main v<version>`. The release workflow (`.github/workflows/release.yml`) gates on
the tests, requires tag == manifest version, and publishes the zip to GitHub Releases
with end-user install notes (generated by `tools/package.mjs`).
